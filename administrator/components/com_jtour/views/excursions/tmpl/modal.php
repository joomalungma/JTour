...
<form action="<?php echo JRoute::_('index.php?option=com_jtour&view=excursions'); ?>" method="post" name="adminForm">
	...
	?>
		<tr class="row<?php echo $k; ?>">
            ...
            <td><a onclick="addExcursion('<?php echo $row->name;?>','<?php echo $row->duration;?>','<?php echo $row->id;?>'); return false;" href="#"><?php echo $row->name != '' ? $row->name : JText::_('JTOUR_NO_TITLE'); ?></a></td>
...
<script type="text/javascript">
    function addExcursion(name,duration,id)
    {
        el =  window.parent.document.getElementById('excursionsTable');
        ...
        tr.align = "center";
        tdName =  window.parent.document.createElement('td');
        tdName.innerHTML = name + '<input type="hidden" name="excursions[]" value="' + id + '"/>';
        tdRate =  window.parent.document.createElement('td');
        tdRate.innerHTML = duration;
        tdRemove =  window.parent.document.createElement('td');
        tdRemove.innerHTML = '<span class="config_remove_btn" onclick="removeExcursion(this);"></span>';
